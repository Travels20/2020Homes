    ...
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center">
            ...
            @php
                $isAdmin = in_array(auth()->user()->role ?? null, ['admin', 'superadmin'], true);
            @endphp
            <a href="{{ $isAdmin ? route('admin.properties.index') : route('admin.my-properties') }}" class="btn btn-outline-secondary">
            ...
            <div class="card-body p-4">
                ...
                <form action="{{ route('admin.properties.update', $property->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4 mb-3">
                            <label class="form-label">Address (Current location)</label>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="latitude" id="latitude" class="form-control" value="{{ old('latitude', $property->latitude) }}" placeholder="e.g. 13.082680">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label">Longitude</label>
                            <input type="text" name="longitude" id="longitude" class="form-control" value="{{ old('longitude', $property->longitude) }}" placeholder="e.g. 80.270718">
                        </div>
                        <div class="col-md-4 mb-3">
                    ...
                    @php
                        ...
                        if (is_string($oldNearby)) {
                            try {
                                ...
                                $oldNearby = [['label' => $oldNearby, 'distance' => '']];
                            }
                        }

                        if (!is_array($oldNearby) || empty($oldNearby)) {
                            $oldNearby = [['label' => '', 'distance' => '']];
                        }
                    ...
                    @php
                        ...
                        if (is_string($oldFaqs)) {
                            try {
                                ...
                                $oldFaqs = [['question' => $oldFaqs, 'answer' => '']];
                            }
                        }

                        if (!is_array($oldFaqs) || empty($oldFaqs)) {
                            $oldFaqs = [['question' => '', 'answer' => '']];
                        }
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="mt-2">
                                <img id="feature_preview" src="{{ $property->feature_image_url }}" alt="Feature Image" class="img-thumbnail" style="height: 150px; width: 50%; object-fit: cover;">
                            </div>
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            @if($property->images && count($property->images) > 0)
                                <div id="existing_gallery" class="d-flex flex-wrap gap-2 mt-2">
                                    @foreach($property->images as $image)
                                        <div class="position-relative" style="width: 150px; height: 150px;">
                                            <img src="{{ $image->image_url }}" alt="Gallery" class="img-thumbnail" style="width: 100%; height: 100%; object-fit: cover;">
                                            <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0" onclick="deletePropertyImage({{ $image->id }})">
                                                <i class="bi bi-x"></i>
                                            </button>
                                        </div>
                                    @endforeach
                            ...
                            @endif
                            <div id="gallery_preview" class="d-flex flex-wrap gap-2 mb-3"></div>
                            <small class="text-secondary">Add additional photos of the property.</small>
                        </div>
                    </div>
                    ...
                    <div class="row mt-4">
                        <div class="col-12">
                            ...
                            @if($isAdmin)
                                ...
                                <div class="form-check mb-3">
                                    <input class="form-check-input" type="checkbox" name="is_verified" id="is_verified" value="1" {{ old('is_verified', $property->is_verified) ? 'checked' : '' }}>
                                    <label class="form-check-label" for="is_verified">
                                        Approved / Verified (Visible on website)
                                    </label>
                                </div>
                            @endif
                            ...
                            <div class="row mb-4">
                                <div class="col-md-4">
                                    <select name="status" class="form-select">
                                        <option value="draft" {{ old('status', $property->status) == 'draft' ? 'selected' : '' }}>Draft (Not visible on website)</option>
                                        <option value="available" {{ old('status', $property->status) == 'available' ? 'selected' : '' }}>Published / Available (Visible on website)</option>
...
function deletePropertyImage(imageId) {
    if (confirm('Are you sure you want to delete this image?')) {
        ...
        form.method = 'POST';
        form.action = `/admin/properties/image/${imageId}`;
        form.innerHTML = '@csrf @method("DELETE")<input type="hidden" name="_token" value="{{ csrf_token() }}">' +
